...
# El nombre de la tabla en la base de datos MySQL
MYSQL_TABLE_NAME = st.secrets["connections"]["mysql"].get("table_name", "gestiones_interg") 
...
def buscar_cedula_en_mysql(cedula_id):
    engine = get_db_connection()
    if engine is None:
        return pd.DataFrame() # No se pudo conectar a la BD
    
    try:
        # **¡IMPORTANTE!** Usamos una consulta parametrizada para prevenir inyecciones SQL
        # y para que pd.read_sql maneje correctamente el tipo de dato.
        query = f"SELECT * FROM {MYSQL_TABLE_NAME} WHERE CEDULA = %s ORDER BY FECHA DESC"
        
        # pd.read_sql puede tomar un segundo argumento 'params' para las consultas parametrizadas
        df_result = pd.read_sql(query, engine, params=(cedula_id,))
        
        if not df_result.empty:
            st.info(f"Información cargada para la cédula: {cedula_id}. Filas: {len(df_result)}")
        else:
            st.warning(f"No se encontró información para la cédula: {cedula_id}.")
            
        return df_result
    except Exception as e:
        st.error(f"Error al buscar el ID {cedula_id} en la base de datos: {e}")
